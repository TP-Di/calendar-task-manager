...
## Теги (важно: разные места для разных тегов):
- Параметр `tag` принимает СТРОГО одно из значений enum схемы: HARD, SOFT, PRIORITY:бакалавр, PRIORITY:работа, PRIORITY:магистратура, PRIORITY:проекты, PRIORITY:курсы. Других значений НЕ передавай — упадёт валидация. Не нужен — не передавай вовсе.
- Метки CATEGORY и DEPENDS — это ТОЛЬКО текст в description, в параметр `tag` их класть НЕЛЬЗЯ.
- При create_event/bulk_create_events ВСЕГДА дописывай в конец description литерал `[CATEGORY:x]` (учёба|работа|дорога|личное). Не заменяй существующий текст description. Если непонятно — `[CATEGORY:личное]`.
- DEPENDS пишем так же: `[DEPENDS:название]` в описание.
Не добавляй эмодзи в начало title при create_task — только чистый текст без эмодзи-префикса.

## Выбор инструмента (встреча vs блок):
- "встреча/созвон/звонок/созвонимся/собеседование/приём/визит" с конкретным временем → `create_event` (в Calendar появляется обычное событие, без 📋 и без задачи).
- "поработаю над X / сяду делать X / выделить N часов на X" с конкретным временем → `create_task` с start_time+end_time (создаст 📋 блок + задачу).
- Сомневаешься — задай ОДИН уточняющий вопрос вместо того чтобы выбирать наугад.
        ...
        try:
            ...
            if "tool_use_failed" in err_str or "failedgeneration" in err_str or "invalid_function_call" in err_str:
                ...
                messages.append({
                    "role": "user",
                    "content": (
                        "Предыдущий вызов инструмента не удался — нарушение схемы. "
                        "Проверь: (1) параметр tag у create_event/bulk_create_events принимает ТОЛЬКО "
                        "значения из enum (HARD, SOFT, PRIORITY:бакалавр/работа/магистратура/проекты/курсы). "
                        "CATEGORY и DEPENDS — только текст в description, в tag их класть нельзя. "
                        "Если подходящего значения нет — параметр tag не передавай. "
                        "(2) event_id/task_id не выдумывай — сначала get_events/get_tasks, потом действие. "
                        "(3) передавай только параметры, перечисленные в схеме инструмента."
                    ),
                })
                continue
