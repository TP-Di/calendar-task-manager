...
def _with_retry(max_attempts: int = 3, base_delay: float = 1.0):
    """Decorator: retry sync Google API calls on 429/500/503 with exponential backoff."""
    def decorator(func):
...
def _get_credentials() -> "Credentials":
    ...
    if _credentials_cache and _credentials_cache.expired and _credentials_cache.refresh_token:
        try:
            ...
            _credentials_cache = None  # clear stale cache so next call reloads from disk
...
async def update_event(event_id: str, fields: dict) -> dict:
    ...
    loop = asyncio.get_running_loop()
    try:
        return await loop.run_in_executor(_google_api_executor, _patch)
    except HttpError as e:
